...
final class ActionList3DemoVC: UIViewController {
    ...
    private let viewModel = ActionList3ViewModel(dependency: .init(fetchUsers: ListItemAPI.fetchUsers))
    ...
    private func setupLayout() {
        hintLabel.text = """
        仍用 action enum，但先拆 viewDidLoadTrigger / refreshTrigger / foregroundTrigger，再 Merge3。
        合流變清楚；confirm 仍要 withLatestFrom（這段 enum 還是彆扭）。
        """
...
extension ActionList3DemoVC: UITableViewDataSource, UITableViewDelegate {
